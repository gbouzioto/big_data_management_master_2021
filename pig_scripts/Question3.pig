...
-- Keep top 3 teams (or more if equal) and top 1 sport
top_athlete_count = FILTER rank_athlete_count BY stitched::result <= 3;
top_sport_count = FOREACH grouped_sport_count {
			ordered = ORDER sport_count BY entries DESC;
			top_sport = LIMIT ordered 1;
			GENERATE FLATTEN(top_sport);
			};
...
final_sport_count = FOREACH top_sport_count GENERATE
			top_sport::games AS games, top_sport::team AS team,
			top_sport::noc AS noc, top_sport::sport AS sport;
...
final_result = ORDER final_result BY games ASC, entries DESC, team ASC; 
